...
ROOT = Path(__file__).resolve().parent.parent          # .../CX5/stock_GPT
DB_PATH = Path(os.environ.get("CX5_STOCK_DB", ROOT / "db" / "portfolio.db"))
SITE_DIR = ROOT                                         # index.html 直接放 stock_GPT/ 底下
                                                        # → https://aaasdream.github.io/CX5/stock_GPT/

# ── 帳戶 ────────────────────────────────────────────────────────────
INITIAL_CASH = 500_000.0      # 新台幣
BASE_CURRENCY = "TWD"
START_DATE = "2026-08-21"     # 比賽第一個交易日
END_DATE = "2026-11-20"       # 11/21 為週六，最後估值採 11/20 收盤
...
# ── 風控（自主交易時的自我約束）────────────────────────────────────
MAX_POSITION_PCT = 0.12       # 單一個股上限：總資產的 12%
MAX_NEW_BUY_PCT = 0.10        # 單筆新進場上限：總資產的 10%
MIN_CASH_PCT = 0.20           # 常態現金水位下限：總資產的 20%
